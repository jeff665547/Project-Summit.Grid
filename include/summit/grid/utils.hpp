...
struct Utils{
    ...
    static auto guess_um2px_r( 
        const cv::Mat_<T>& src, 
        const nlohmann::json& chipinfo 
    ) {
        auto u8_src = chipimgproc::norm_u8(src);
        auto& chip_spec = chipinfo["spec"];
    ...
    static auto generate_sgl_pat_reg_mat_marker_layout(
        ...
        auto& spec              = chipinfo["spec"];
        auto& shooting_marker   = spec["shooting_marker"];
        auto& position_cl       = shooting_marker["position_cl"];
        auto& position          = shooting_marker["position"];
        auto& fov_cl            = chipinfo["fov_cl"];

        auto fov_rows = fov["rows"].get<int>();
        auto fov_cols = fov["cols"].get<int>();
    ...
    static auto generate_stitch_points(
        ...
        auto& fov_cl = chipinfo["fov_cl"];
        auto& fov = chipinfo["fov"];
        auto rows = fov["rows"].get<int>();
        auto cols = fov["cols"].get<int>();
