...
struct Paths {
    ...
    /**
     * @brief Create the WARNING file (juduged automatically by the algorithm) to remind user to double-check the gridding result.
     * 
     * @param task_id The task ID is generated by summit::app::grid::TaskID
     */
    void create_warning_file(
        const TaskID& task_id
    ) const {
        for(auto&& p : {
            // input WARNING
            task_id.path() / "grid" / "WARNING",
            // output WARNING
            task_output(task_id) / "grid" / "WARNING"
        }){
            std::ofstream fout(check_path(p).string(), std::fstream::trunc | std::fstream::out);
            fout.close();
        }
